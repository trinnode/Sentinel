...
export class WebhookService {
  ...
  async sendWebhook(
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        const req = (isHttps ? https : http).request(options, (res) => {
          ...
          res.on("end", () => {
            if (
              ...
              console.log(
                `Webhook ${webhook.name} delivered successfully (${res.statusCode})`
              );
              resolve(true);
            } else {
              console.error(
                `Webhook ${webhook.name} failed with status ${res.statusCode}: ${data}`
              );
        ...
        req.on("error", (error) => {
          console.error(
            `Webhook ${webhook.name} request error:`,
            error.message
          );
          reject(error);
        });
  ...
  // Send test webhook
  async sendTestWebhook(webhookId: string, userId: string): Promise<boolean> {
    try {
  ...
  async processDeliveryResult(
    ...
    try {
      ...
      if (success) {
        console.log(
          `Webhook ${webhookId} delivered successfully (${responseCode})`
        );
      } else {
  ...
  async sendBulkWebhooks(
    ...
    try {
      ...
      console.log(
        `Sending ${event} to ${webhooks.length} webhooks for user ${userId}`
      );
