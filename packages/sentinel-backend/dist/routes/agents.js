...
const createAgentSchema = zod_1.z.object({
    ...
    validatorId: zod_1.z.string().min(1, "Validator ID is required"),
});
const updateAgentSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, "Name is required").optional(),
    isActive: zod_1.z.boolean().optional(),
    validatorId: zod_1.z.string().min(1, "Validator ID is required").optional(),
});
...
router.get("/", async (req, res) => {
    try {
        ...
        const agents = await app_1.prisma.agent.findMany({
            where: {
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const agent = await app_1.prisma.agent.findUnique({
...
router.post("/", async (req, res) => {
    try {
        ...
        // Create agent
        const agent = await app_1.prisma.agent.create({
            data: {
                apiKey,
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (updateData.validatorId) {
            const targetValidator = await app_1.prisma.validator.findFirst({
                where: {
        ...
        // Update agent
        const updatedAgent = await app_1.prisma.agent.update({
            where: { id: req.params.id },
            data: updateData,
            include: {
                validator: {
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        await app_1.prisma.agent.delete({
            where: { id: req.params.id },
        });
        res.json({
            success: true,
...
router.get("/:id/reports", async (req, res) => {
    try {
        ...
        const reports = await app_1.prisma.agentReport.findMany({
