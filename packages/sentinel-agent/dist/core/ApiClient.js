...
class ApiClient {
    ...
    setupInterceptors() {
        ...
        this.client.interceptors.response.use((response) => {
            this.logger.logApiRequest(response.config.url || '', response.config.method || 'GET', response.status);
            return response;
        }, (error) => {
            if (error.response) {
                this.logger.logApiRequest(error.config?.url || '', error.config?.method || 'GET', error.response.status, error.response.data?.error || error.message);
            }
            else {
    ...
    async updateAgentStatus(status, metadata) {
        try {
