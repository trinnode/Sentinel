...
const BeaconNodeMonitor_1 = require("../monitoring/BeaconNodeMonitor");
const ApiClient_1 = require("./ApiClient");
const PeerNetwork_1 = require("../p2p/PeerNetwork");
class SentinelAgent {
    constructor() {
        this.isRunning = false;
        this.monitoringInProgress = false;
        this.handleIncomingConsensusRequest = async (request) => {
            if (!this.peerNetwork) {
                return;
            }
            ...
            try {
                const latestHealth = this.lastHealthCheck || (await this.beaconMonitor.manualHealthCheck());
                ...
                this.logger.logConsensusEvent(request.validatorId, "consensus_response_prepared", {
                ...
                this.logger.error("Failed to handle incoming consensus request", error instanceof Error ? error : undefined, {
                    validatorId: request.validatorId,
                });
            }
        };
        ...
        this.beaconMonitor = new BeaconNodeMonitor_1.BeaconNodeMonitor();
    ...
    async start() {
        ...
        try {
            ...
            this.logger.error("Failed to start Sentinel Agent", error instanceof Error ? error : undefined);
            throw error;
        }
    }
    async stop() {
    ...
    async initializePeerNetwork() {
        ...
        const peerNetwork = new PeerNetwork_1.PeerNetwork();
        ...
        try {
            ...
            this.logger.error("Failed to initialize peer network", error instanceof Error ? error : undefined);
            throw error;
        }
    }
    async teardownPeerNetwork() {
        if (!this.peerNetwork) {
            return;
        }
        this.peerNetwork.off("consensus_request", this.handleIncomingConsensusRequest);
        try {
            await this.peerNetwork.stop();
        }
        catch (error) {
            this.logger.error("Failed to stop peer network", error instanceof Error ? error : undefined);
        }
        finally {
    ...
    async performMonitoringCycle() {
        ...
        try {
            ...
            this.logger.error("Error in monitoring cycle", error instanceof Error ? error : undefined);
        }
        finally {
            this.monitoringInProgress = false;
        }
    }
    async handleHealthResult(healthResult) {
        ...
        if (shouldReportUnhealthy) {
            const consensusConfirmed = await this.confirmUnhealthyWithPeers(healthResult);
            if (!consensusConfirmed) {
                this.logger.warn("Consensus threshold not met; skipping unhealthy report", {
    ...
    async confirmUnhealthyWithPeers(healthResult) {
        ...
        try {
            ...
            this.logger.logConsensusEvent(healthResult.validatorId, "consensus_result", {
            ...
            if (consensusResult.totalPeers === 0) {
                this.logger.warn("No peers responded to consensus request; proceeding with local report", {
                    validatorId: healthResult.validatorId,
                });
                return true;
            }
            ...
            this.logger.error("Consensus confirmation failed", error instanceof Error ? error : undefined, {
                validatorId: healthResult.validatorId,
            });
            return false;
        }
    ...
    async sendHealthReport(healthResult) {
        const payload = {
            agentId: this.config.getAgentId(),
            validatorId: healthResult.validatorId,
            status: healthResult.status,
        ...
        try {
            ...
            this.logger.error("Error sending health report", error instanceof Error ? error : undefined);
        }
    }
    async updateAgentStatus(healthResult) {
        try {
    ...
    async performManualHealthCheck() {
        ...
        try {
            ...
            this.logger.error("Manual health check failed", error instanceof Error ? error : undefined);
            throw error;
        }
    }
    getStatus() {
